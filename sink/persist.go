...
func Persist(ctx context.Context) error {
	...
	err = tx.Where("UpdatedHeight >= ? AND UpdatedHeight <= ?", heightMin, heightMax).Find(&existingActors).Error
	if err != nil {
		return err
	}
	...
	err = tx.Where("UpdatedHeight >= ? AND UpdatedHeight <= ?", heightMin, heightMax).Find(&existingMiners).Error
	if err != nil {
		return err
	}
	...
	err = tx.Where("UpdatedHeight >= ? AND UpdatedHeight <= ?", heightMin, heightMax).Find(&existingAsks).Error
	if err != nil {
		return err
	}
